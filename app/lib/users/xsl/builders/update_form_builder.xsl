...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:php="http://php.net/xsl">
    ...
    <xsl:template match="/">
        <form action="/users/update/props" method='POST' enctype="multipart/form-data" name='formname' id='form_id' class='form'>
    ...
    <xsl:template name="address_object_id_">
        <input id="address_field_hidden" type="text" style="display: none;">
            ...
            <xsl:attribute name="value"><xsl:value-of select="php:function('User::getPropsValue',string(column_name),'mask')" /></xsl:attribute>
        </input>
        <input id="address_field" type="text">
            <xsl:attribute name="value"><xsl:value-of select="php:function('User::getPropsValue',string(column_name),'value')" /></xsl:attribute>
